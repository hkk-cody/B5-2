...
handler의 `_format_log_commit()`은 각 커밋을 직접 가리키는 브랜치도 찾아 출력에
붙입니다. 브랜치는 과거 커밋 전체에 표시되는 것이 아니라 현재 그 해시를 직접
가리킬 때만 표시됩니다.
...
| 부모 우선 로그 | Kahn 위상 정렬과 최소 힙 | O((V + E) log V) | `LOG` |
| 날짜·작성자 로그 | Merge Sort | O(N log N) | `LOG --sort-by=...` |
| 최단 경로 | 양쪽 거리 BFS와 경로 복원 | O(V + E) | `PATH` |
| 모든 조상 | 스택 기반 그래프 탐색 | O(V + E) | `ANCESTORS` |
