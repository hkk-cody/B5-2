...
최단 경로를 벗어날 수 있습니다. 길이가 같은 40자리 전체 해시에서는 처음 다른 해시의 비교로
...
통과 횟수보다 각 테스트가 무엇을 검증하는지 읽어보세요.
...
- 충돌 테스트: `patch()`로 SHA-1 반환값을 강제합니다. 40자리 전체가 같은 결과를 두 번 주고
  세 번째에 다른 해시를 반환해 재시도·기존 커밋 보존·인덱스 갱신을 확인합니다.
- 접두어 테스트: 앞 6자리만 같은 해시는 각각 저장되는지, 표시 길이가 늘어나는지,
  축약 입력으로 전체 커밋을 찾는지, 모호한 입력은 거부하는지 확인합니다.
